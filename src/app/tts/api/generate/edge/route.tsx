...
const APPLICATION_X_WWW_FORM_URLENCODED_MIME_TYPE =
  ...
  const stream = new ReadableStream({
    start(controller) {
      async function read() {
        ...
        if (firstRead) {
          console.log(
            `${timer.startTime} TTS first byte latency: ${timer.elapsedString} ms`,
          );
          firstRead = false;
        }
        if (done) {
          console.log(
            `${timer.startTime} TTS complete latency: ${timer.elapsedString} ms`,
          );
          controller.close();
          return;
        }
...
export async function GET(request: NextRequest) {
  ...
  if (!providerName || !voice || !text) {
    return new NextResponse(
      JSON.stringify({
        error: "You must specify params `provider`, `text`, and `voice`.",
      }),
      {
        status: 400,
      },
  ...
  if (!response.ok) {
    ...
    console.log(
      `${timer.startTime} TTS error: ${response.status} ${response.statusText}`,
    );
  ...
  if (provider.keyPath) {
    if (!contentType?.startsWith(APPLICATION_JSON_MIME_TYPE)) {
      console.warn(
        `${timer.startTime} TTS expected JSON response, got ${contentType}`,
      );
    }
    const binary = getBlobFromJson(
      timer,
      await response.json(),
      provider.keyPath,
    );
...
function ttsCoqui({ text, voice, rate }: GenerateOptions): Promise<Response> {
  const headers = createHeaders({
    authorization: makeAuth("COQUI_API_KEY"),
...
function ttsDeepgram({
  ...
  const url = `https://api.beta.deepgram.com/v1/speak?${model_string}&encoding=mp3&bit_rate=32000`;
  const obj = { text };
  return postJson(url, headers, obj);
}
...
function ttsLmnt({ text, voice, rate }: GenerateOptions): Promise<Response> {
  const headers = createHeaders({
    x_api_key: getEnvVar("LMNT_API_KEY"),
    accept: AUDIO_WAV_MIME_TYPE,
  });
...
function ttsMurf({ text, voice, rate }: GenerateOptions): Promise<Response> {
  const headers = createHeaders({
    api_key: getEnvVar("MURF_API_KEY"),
...
function ttsOpenAI({
  ...
  const headers = createHeaders({
    authorization: makeAuth("OPENAI_API_KEY"),
...
function ttsPlayHT({
  ...
  const headers = createHeaders({
    authorization: makeAuth("PLAYHT_API_KEY"),
    accept: AUDIO_MPEG_MIME_TYPE,
  });
...
function ttsResembleV1({
  ...
  const url = `https://app.resemble.ai/api/v2/projects/${getEnvVar(
    "RESEMBLE_PROJECT_ID",
  )}/clips`;
  return postJson(url, headers, obj);
}
