...
export class AudioOutputManager extends EventTarget {
  ...
  private readonly streams: Map<string, AudioStream> = new Map<
    string,
    AudioStream
  >();
  async start() {
    ...
    console.log(
      `AudioOutputManager starting, sample rate=${this.context.sampleRate}`,
    );
  ...
  createStream(wantAnalyzer = true) {
    ...
    audioStream.onPlaying = (streamId) =>
      this.handleStreamStateChange(streamId, true);
    audioStream.onWaiting = (streamId) =>
      this.handleStreamStateChange(streamId, false);
...
export abstract class WebAudioTextToSpeech extends TextToSpeechBase {
  ...
  constructor(name: string, voice: string, rate: number, model?: string) {
    ...
    this.audio.onloadstart = () =>
      console.debug(`[${this.name}] tts loadstart`);
    this.audio.onloadeddata = () =>
      console.debug(`[${this.name}] tts loadeddata`);
  ...
  play(text: string) {
    if (this.audio.readyState == 0 || this.audio.paused) {
      ...
      outputManager.addEventListener(
        "playing",
...
export class RestTextToSpeech extends WebAudioTextToSpeech {
  ...
  async warmup() {
    ...
    console.log(
      `[${this.name}] warmup complete, elapsed=${elapsed.toFixed(0)} ms`,
    );
  }
  ...
  private async dispatchRequest(req: TextToSpeechRequest) {
    if (req.cancelled) {
      console.log(
        `[${this.name}] ignoring cancelled request: ${req.shortText}`,
      );
      return;
    }
    ...
    if (!res.ok) {
      ...
      this.setComplete(
        new Error(
          `[${this.name}] generation request failed: ${res.status} ${res.statusText}`,
        ),
      );
      return;
    }
    ...
    while (true) {
      ...
      console.debug(
        `[${this.name}] received chunk buffer: ${req.shortText}, len=${value.length}`,
      );
...
export class AwsTextToSpeech extends RestTextToSpeech {
  ...
  constructor(
    urlFunc: BuildUrl,
    voice: string = AwsTextToSpeech.DEFAULT_VOICE,
    rate: number = 1.0,
  ) {
...
export class AzureTextToSpeech extends RestTextToSpeech {
  ...
  constructor(
    urlFunc: BuildUrl,
    voice: string = AzureTextToSpeech.DEFAULT_VOICE,
    rate: number = 1.0,
  ) {
...
export class CoquiTextToSpeech extends RestTextToSpeech {
  ...
  constructor(
    urlFunc: BuildUrl,
    voice: string = CoquiTextToSpeech.DEFAULT_VOICE,
    rate: number = 1.0,
  ) {
 ...
 * Text-to-speech implementation that uses the Google Cloud text-to-speech service.
 */
export class DeepgramTextToSpeech extends RestTextToSpeech {
  static readonly DEFAULT_VOICE = "asteria";
  constructor(
    urlFunc: BuildUrl,
    voice: string = DeepgramTextToSpeech.DEFAULT_VOICE,
    rate: number = 1.0,
  ) {
    super("deepgram", urlFunc, voice, rate);
  }
}
...
export class GcpTextToSpeech extends RestTextToSpeech {
  ...
  constructor(
    urlFunc: BuildUrl,
    voice: string = GcpTextToSpeech.DEFAULT_VOICE,
    rate: number = 1.0,
  ) {
...
export class MurfTextToSpeech extends RestTextToSpeech {
  ...
  constructor(
    urlFunc: BuildUrl,
    voice: string = MurfTextToSpeech.DEFAULT_VOICE,
    rate: number = 1.0,
  ) {
...
export class OpenAITextToSpeech extends RestTextToSpeech {
  ...
  constructor(
    urlFunc: BuildUrl,
    voice: string = OpenAITextToSpeech.DEFAULT_VOICE,
    rate: number = 1.0,
  ) {
...
export class PlayHTTextToSpeech extends RestTextToSpeech {
  ...
  constructor(
    urlFunc: BuildUrl,
    voice: string = PlayHTTextToSpeech.DEFAULT_VOICE,
    rate: number = 1.0,
  ) {
...
export class ResembleTextToSpeech extends RestTextToSpeech {
  ...
  constructor(
    urlFunc: BuildUrl,
    voice: string = ResembleTextToSpeech.DEFAULT_VOICE,
    rate: number = 1.0,
  ) {
...
export class WellSaidTextToSpeech extends RestTextToSpeech {
  ...
  constructor(
    urlFunc: BuildUrl,
    voice: string = WellSaidTextToSpeech.DEFAULT_VOICE,
    rate: number = 1.0,
  ) {
...
export abstract class WebSocketTextToSpeech extends WebAudioTextToSpeech {
  ...
  protected ensureSocket() {
    ...
    socket.onopen = async (_event) => {
      ...
      console.log(
        `[${this.name}] socket opened, elapsed=${elapsed.toFixed(0)}`,
      );
    ...
    socket.onmessage = (event) => {
      ...
      if (typeof event.data == "string") {
        try {
          ...
          console.error(
            `Failed to parse socket message: ${error}, data=${event.data}`,
          );
          return;
        }
...
export function createTextToSpeech({
  ...
  if (!proto || proto == "rest") {
    switch (provider) {
      ...
      case "deepgram":
        return new DeepgramTextToSpeech(buildUrl!, voice, rate);
      case "eleven":
