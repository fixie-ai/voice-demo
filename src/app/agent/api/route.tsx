...
import {
  OpenAI,
  ValidChatModel as OpenAIValidChatModel,
} from "ai-jsx/lib/openai";
import {
  Anthropic,
  ValidChatModel as AnthropicValidChatModel,
} from "ai-jsx/lib/anthropic";
...
async function ChatAgent({
  ...
  if (model.startsWith("gpt-")) {
    return (
      <OpenAI chatModel={model as OpenAIValidChatModel}>{children}</OpenAI>
    );
  }
  if (model.startsWith("claude-")) {
    return (
      <Anthropic chatModel={model as AnthropicValidChatModel}>
        {children}
      </Anthropic>
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  json.messages.forEach((message: ClientMessage) =>
