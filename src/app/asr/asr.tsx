"use client";
import { LibfVoiceActivityDetector, VoiceActivityDetectorBase } from "./vad";
...
export class MicManager extends EventTarget {
  ...
  private async startGraph(
    ...
    console.log(
      `MicManager starting graph, sample rate=${this.context.sampleRate}`,
    );
    ...
    this.vad.onVoiceStart = () => {
      ...
      this.dispatchEvent(
        new CustomEvent("vad", { detail: new VoiceActivity(true, now) }),
      );
    };
    this.vad.onVoiceEnd = () => {
      const now = this.currentMillis;
      ...
      this.dispatchEvent(
        new CustomEvent("vad", {
          detail: new VoiceActivity(false, now, duration),
        }),
      );
    };
    this.vad.start();
...
export abstract class SpeechRecognitionBase extends EventTarget {
  ...
  protected startInternal(url: string, protocols?: string[]) {
    ...
    this.manager.addEventListener(
      "chunk",
      (evt: CustomEventInit<ArrayBuffer>) => {
    ...
    this.manager.addEventListener(
      "vad",
      (evt: CustomEventInit<VoiceActivity>) => {
        const update = evt.detail!;
        if (!update.active) {
          if (
            !this.streamLastVoiceActivity ||
            update.duration! > this.streamLastVoiceActivity.duration!
          ) {
            console.log(
              `[${this.name}] voice activity end ts=${update.timestamp} duration=${update.duration} ms`,
            );
  ...
  protected dispatchTranscript(
    transcript: string,
    final: boolean,
    recognizedMillis: number,
  ) {
    if (!final && recognizedMillis < this.streamRecognizedMillis) {
      console.warn(
        `[${this.name}] recognition time ${recognizedMillis} < ${this.streamRecognizedMillis}, ignoring`,
      );
      return;
    }
    ...
    if (final) {
      if (this.streamLastVoiceActivity) {
        observedLatency =
          this.manager.currentMillis - this.streamLastVoiceActivity.timestamp;
        this.streamLastVoiceActivity = undefined;
      } else {
        console.warn(
          `[${this.name}] no voice activity end detected prior to final transcript`,
        );
      }
    }
  ...
  protected flush() {
    if (this.outBuffer.length > 0) {
      const bufMillis =
        (this.outBuffer.length * AUDIO_WORKLET_NUM_SAMPLES * 1000) /
        this.manager.sampleRate;
  ...
  private concatWords(words: any[]) {
    ...
    if (transcript.endsWith(SonioxSpeechRecognition.END_TOKEN)) {
      transcript = transcript.slice(
        0,
        -SonioxSpeechRecognition.END_TOKEN.length,
      );
      done = true;
    }
...
export class AssemblyAISpeechRecognition extends SpeechRecognitionBase {
  ...
  protected handleMessage(result: any) {
    if (
      result.message_type == "PartialTranscript" ||
...
export class SpeechmaticsSpeechRecognition extends SpeechRecognitionBase {
  ...
  async start() {
    ...
    super.startInternal(
      `wss://eu.rt.speechmatics.com/v2/${languageCode}?jwt=${await this.fetchToken()}`,
    );
  }
  ...
  protected handleMessage(result: any) {
    if (result.message == "AddPartialTranscript") {
      ...
      result.message != "AudioAdded" &&
      result.message != "RecognitionStarted" &&
      result.message != "Info"
    ) {
...
export class RevAISpeechRecognition extends SpeechRecognitionBase {
  ...
  protected handleMessage(result: any) {
    ...
    if (result.type == "partial") {
      const text = result.elements.reduce(
        (t: string, w: any) => `${t}${w.value} `,
        "",
      );
